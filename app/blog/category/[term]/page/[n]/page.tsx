...
/**
 * `/blog/category/{term}/page/{n}` -- the paginated category archive (Phase
 * 22, Plan 06, BLOG-07, D-3/D-5).
 *
 * A directory named `page` and a file named `page.tsx` coexist in the
 * PARENT folder (`app/blog/category/[term]/`) by design -- same reasoning as
 * `app/blog/page/[n]/page.tsx`'s own doc comment.
 *
 * `n=1` redirects to the bare `/blog/category/{term}` path -- D-5's page-1
...
interface CategoryArchivePaginatedPageProps {
  params: Promise<{ term: string; n: string }>;
}

export async function generateMetadata({ params }: CategoryArchivePaginatedPageProps) {
  const { term, n } = await params;
  const page = parseBlogPageParam(n);
  if (page === null) return { title: NOT_FOUND_TITLE };
  return buildArchiveMetadata({ kind: "category", termSlug: term, page });
}

export default async function BlogCategoryArchivePaginatedPage({
  params,
}: CategoryArchivePaginatedPageProps) {
  const { term, n } = await params;
  const page = parseBlogPageParam(n);
  if (page === null) notFound();
  if (page === 1) redirect(resolveBlogPaginationPath("category", term, 1));
  return <RenderArchive kind="category" termSlug={term} page={page} />;
}
